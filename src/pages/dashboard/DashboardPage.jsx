...
const officerActions = [
  {
    key: 'Review Overdue',
    description: 'Follow up on temporary borrows past their return date.',
    tone: 'rose',
    to: '/app/borrowings?filter=overdue'
  },
  {
    key: 'Assign Equipment',
    description: 'Record long-term laptops and devices issued to staff.',
    tone: 'violet',
    to: '/app/borrowings?action=assign'
  },
  {
    key: 'Record Returns',
    description: 'End assignments or mark temporary returns.',
    tone: 'emerald',
    to: '/app/borrowings'
  }
]
...
const staffActions = [
  {
    key: 'Submit Borrow Request',
    description: 'Request ICT equipment for your department.',
    tone: 'amber',
    to: '/app/borrow-requests?action=submit'
  },
  {
    key: 'Track My Requests',
    description: 'View the status of your pending and past requests.',
    tone: 'sky',
  ...
  return (
    <div>
      ...
      <div className="mt-5 grid gap-4 sm:mt-6 lg:grid-cols-5">
        ...
        <div className={`rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5 ${isOfficer ? 'lg:col-span-2' : 'lg:col-span-5 max-w-2xl'}`}>
          <div className="text-sm font-semibold text-white">Action Center</div>
          <div className="text-sm text-white/60 mt-1">
            {isOfficer ? 'Common next steps for ICT officers.' : 'Quick links for your borrowing requests.'}
          </div>

          <div className="mt-4 space-y-3">
            {(isOfficer ? officerActions : staffActions).map((action) => (
              <Link
                key={action.key}
                to={action.to}
                className="block rounded-xl border border-white/10 bg-black/20 px-4 py-3 transition hover:border-brandAmber-500/30 hover:bg-black/30"
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <div className="text-sm text-white/85 font-medium">{action.key}</div>
                    <div className="text-xs text-white/55 mt-1">{action.description}</div>
                  </div>
                  <span className={`mt-1 inline-block h-2 w-2 shrink-0 rounded-full ${toneClass(action.tone)}`} />
                </div>
              </Link>
            ))}
          </div>
